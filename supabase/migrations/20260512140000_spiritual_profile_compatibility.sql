-- Spiritual profile depth + verification placeholders + contact fields.
-- Contact fields remain readable only via get_matched_contact_fields (security definer).
...
-- Reveal WhatsApp / call preference only when a mutual matched row exists.
